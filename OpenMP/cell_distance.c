...
void distance(int input_x_1, int input_x_2, int input_y_1, int input_y_2, int input_z_1, int input_z_2) {
  ...
  //converts value to index of matrix
  //float test = sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1));
  //index = (int)((roundf(sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1)) * 100))+1);
  index = (int)((sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1)) * 100)+1.5);
  //index = (sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1)) * 100)+1;
  //index = (sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1)));
  //index2 = float_index(sqrt((x_2-x_1)*(x_2-x_1)+(y_2-y_1)*(y_2-y_1)+(z_2-z_1)*(z_2-z_1)));
  //printf("%f %d\n", index, index2);
...
//   int ip = input*1000;
//   if(((ip)%10) > 4){
//     return (int)(((input*1000 + 5)/10)-1000)+1;
//   }
//     return (int) (((input*1000)/10-1000)+1);
// }

    //int coord = FLOAT_TO_FIXED(string_to_float(coordinates));
    //printf("%f\n", FIXED_TO_FLOAT(coord));
